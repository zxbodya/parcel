...
import { ConfigResult, File, FilePath, PackagedBundle, HTTPSOptions, PluginOptions, Blob, Glob, NamedBundle, BundleGraph, Bundle, Dependency, Async, SourceLocation } from '@parcel/types';
...
declare type CreateHTTPServerOpts = {
    ...
    outputFS: FileSystem;
    cacheDir: FilePath;
    listener?: (b: unknown, a: unknown) => void;
    host?: string;
};
...
declare function throttle<TArgs extends Iterable<unknown>>(fn: (...args: TArgs) => unknown, delay: number): (...args: TArgs) => void;
...
declare function findAlternativeFiles(fs: FileSystem, fileSpecifier: string, dir: string, projectRoot: string, leadingDotSlash?: boolean): Promise<Array<string>>;
...
declare function isGlobMatch(filePath: FilePath, glob: Glob): any;
declare function globSync(p: FilePath, fs: FileSystem, options?: Options): Array<FilePath>;
declare function glob(p: FilePath, fs: FileSystem, options: Options): Promise<Array<FilePath>>;
...
declare function replaceURLReferences({ bundle, bundleGraph, contents, map, relative, }: {
    bundle: NamedBundle;
...
export { AnsiDiagnosticResult, AssetStats, BuildMetrics, BundleStats, ConfigOptions, ConfigOutput, DefaultMap, DefaultWeakMap, Deferred, HTTPServer, PathOptions, PromiseQueue, SOURCEMAP_EXTENSIONS, SOURCEMAP_RE, SchemaAllOf, SchemaAny, SchemaArray, SchemaBoolean, SchemaEntity, SchemaEnum, SchemaError, SchemaNot, SchemaNumber, SchemaObject, SchemaOneOf, SchemaString, TapStream, ansiHtml, blobToBuffer, blobToStream, blobToString, bufferStream, countLines, createDependencyLocation, createHTTPServer, debounce, escapeHTML, fallbackStream, findAlternativeFiles, findAlternativeNodeModules, fuzzySearch, generateBuildMetrics, generateCertificate, getCertificate, getRootDir, glob, globSync, hashFile, hashObject, hashStream, isAbsolute, isDirectoryInside, isGlob, isGlobMatch, isURL, loadConfig, loadSourceMap, loadSourceMapUrl, makeDeferredWithPromise, matchSourceMappingURL, measureStreamLength, normalizePath, normalizeSeparators, objectHash, objectSortedEntries, objectSortedEntriesDeep, openInBrowser, prettifyTime, prettyDiagnostic, readableFromStringOrBuffer, relativeBundlePath, relativePath, relativeUrl, remapSourceLocation, replaceInlineReferences, replaceURLReferences, resolveConfig, resolveConfigSync, setDifference, streamFromPromise, throttle, unique, urlJoin, validateSchema };
