...
declare type VisitorFunc<N, S> = (c: N, b: S, a: Array<t.Node>) => void | t.Node | Array<t.Node> | symbol | (() => void | t.Node | Array<t.Node> | symbol);
declare type SingleVisitor<N, S> = VisitorFunc<N, S> | {
    enter?: VisitorFunc<N, S>;
    exit?: VisitorFunc<N, S>;
};
declare type Visitors<S> = {
    ...
    BooleanLiteralTypeAnnotation?: SingleVisitor<t.BooleanLiteralTypeAnnotation, S>;
    ...
    PipelinePrimaryTopicReference?: SingleVisitor<t.PipelinePrimaryTopicReference, S>;
    ...
    TSConstructSignatureDeclaration?: SingleVisitor<t.TSConstructSignatureDeclaration, S>;
    ...
    TSExpressionWithTypeArguments?: SingleVisitor<t.TSExpressionWithTypeArguments, S>;
    ...
    TSNamespaceExportDeclaration?: SingleVisitor<t.TSNamespaceExportDeclaration, S>;
    TSTypeAnnotation?: SingleVisitor<t.TSTypeAnnotation, S>;
    TSTypeParameterInstantiation?: SingleVisitor<t.TSTypeParameterInstantiation, S>;
...
declare type VisitorsExploded<F> = {
    shouldSkip?: (a: t.Node) => boolean;
} & {
    [x: string]: {
        enter?: Array<F>;
...
declare class Path {
    ...
    _skipped: boolean;
    _removed: boolean;
    constructor(node: Node, parent: Node, listkey: string | undefined | null, key: number | string);
    replaceWith(n: Node): void;
    skip(): void;
    remove(): void;
}
declare function traverse<T>(node: Node, visitors: SimpleVisitors<(b: Path, a: T) => void>, state: T): void;

declare const SKIP: symbol;
declare const REMOVE: symbol;
declare function traverse2<T>(node: Node, visitors: Visitors<T> | VisitorsExploded<VisitorFunc<Node, T>>, state: T): void;
declare function mergeVisitors<T, U>(a: Visitors<T>, b: Visitors<U>): VisitorsExploded<VisitorFunc<Node, T & U>>;

declare function traverseAll(node: Node, visitor: (node: Node) => void): void;

declare type ScopeType = 'program' | 'function' | 'arrow_function' | 'block';
declare type BindingType = 'var' | 'let' | 'const';
declare class Scope {
    type: ScopeType;
    names: Set<string>;
    bindings: Map<string, Node>;
    references: Map<string, Set<Identifier>>;
    ...
    addBinding(name: string, decl: Node, type: BindingType | 'param'): void;
    getBinding(name: string): Node | undefined | null;
    addReference(identifier: Identifier): void;
    rename(from: string, to: string): void;
    exit(): void;
}
declare type ScopeState = {
    scope: Scope;
};
declare let scopeVisitor: Visitors<ScopeState>;

declare function simple<T>(node: Node, _visitors: SimpleVisitors<(b: any, a: T) => void>, state: T): void;
declare function ancestor<T>(node: Node, _visitors: SimpleVisitors<(c: any, b: T, a: Array<Node>) => void>, state: T): void;
declare function recursive<T>(node: Node, _visitors: SimpleVisitors<(b: any, a: T, recurse: (a: Node) => void) => void>, state: T): void;

export { BindingType, REMOVE, SKIP, Scope, ScopeState, ScopeType, SimpleVisitors, VisitorFunc, Visitors, VisitorsExploded, ancestor, mergeVisitors, recursive, scopeVisitor, simple, traverse, traverse2, traverseAll };
